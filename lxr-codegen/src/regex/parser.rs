...
use crate::regex::expression::Expression;
...
impl<'a> Parser<'a> {
    ...
    fn parse_alternation(&mut self) -> Result<Expression, ParseError> {
        let mut alternation = self.parse_concatenation()?;
        while self.cursor.accept('|') {
            alternation = alternation.alternate(self.parse_concatenation()?);
        }
        Ok(alternation)
    }

    fn parse_concatenation(&mut self) -> Result<Expression, ParseError> {
        let mut sequence = Expression::Epsilon;
        while !self.at_concatenation_end() {
            sequence = sequence.concat(self.parse_quantified()?);
        }
        Ok(sequence)
    }

    fn parse_quantified(&mut self) -> Result<Expression, ParseError> {
        let atom = self.parse_atom()?;
        let Some(quantifier) = self.parse_quantifier()? else {
            return Ok(atom);
        };
    ...
    fn parse_atom(&mut self) -> Result<Expression, ParseError> {
        ...
        match self.cursor.peek() {
            ...
            Some('[') => Ok(Expression::Class(self.parse_class()?)),
            Some('.') => {
                self.cursor.pop();
                Ok(Expression::Class(CharSet::single('\n').negate()))
            }
            Some('\\') => Ok(Expression::Class(self.parse_escape()?.into_set())),
            Some(quantifier @ ('*' | '+' | '?')) => {
            ...
            Some(literal) => {
                self.cursor.pop();
                Ok(Expression::Class(CharSet::single(literal)))
            }
        }
    }

    fn parse_repetition(&mut self) -> Result<Option<Quantifier>, ParseError> {
        let checkpoint = self.cursor.clone();
        let position = self.position();
        self.cursor.pop();
        match self.parse_repetition_bounds(position)? {
            Some(quantifier) => Ok(Some(quantifier)),
            None => {
    ...
    fn parse_repetition_bounds(
        &mut self,
        repetition_position: usize,
    ) -> Result<Option<Quantifier>, ParseError> {
        let Some(minimum) = self.parse_bound()? else {
            return Ok(None);
        };
        if self.cursor.accept('}') {
            return Ok(Some(Quantifier::exactly(minimum)));
        }
        if !self.cursor.accept(',') {
            return Ok(None);
        }
        if self.cursor.accept('}') {
            return Ok(Some(Quantifier::at_least(minimum)));
        }
        ...
        Quantifier::range(minimum, maximum).map(Some).map_err(|_| {
            self.spanned(
                Kind::InvertedRepetition { minimum, maximum },
                repetition_position,
            )
        })
    }
    ...
    fn at_concatenation_end(&self) -> bool {
...
mod tests {
    ...
    fn single(character: char) -> Expression {
        Expression::Class(CharSet::single(character))
    }

    fn class(set: CharSet) -> Expression {
        Expression::Class(set)
    }
    ...
    fn literals_after(node: Expression, text: &str) -> Expression {
        text.chars()
            .fold(node, |node, character| node.concat(single(character)))
    }

    fn literals(text: &str) -> Expression {
        literals_after(Expression::Epsilon, text)
    }

    fn class_of(pattern: &str) -> CharSet {
        match parse(pattern) {
            Ok(Expression::Class(set)) => set,
    ...
    fn empty_pattern_parses_to_epsilon() {
        assert_eq!(parse(""), Ok(Expression::Epsilon));
    }
    ...
    fn empty_alternation_branch_parses_to_epsilon() {
        let expected = single('a').alternate(Expression::Epsilon);
    ...
    fn exact_repetition_parses_to_an_equal_range() {
        let expected = single('a').repeated(Quantifier::exactly(3));
        assert_eq!(parse("a{3}"), Ok(expected));
    }

    #[test]
    fn open_ended_repetition_parses_to_at_least() {
        let expected = single('a').repeated(Quantifier::at_least(3));
        assert_eq!(parse("a{3,}"), Ok(expected));
    }

    #[test]
    fn bounded_repetition_parses_to_a_range() {
        let expected = single('a').repeated(Quantifier::range(3, 5).unwrap());
    ...
    fn repetition_bound_above_the_pcre_limit_is_rejected() {
        let expected = single('a').repeated(Quantifier::exactly(65535));
    ...
    fn quantifiers_apply_to_every_kind_of_atom() {
        ...
        let expected = group.repeated(Quantifier::range(2, 3).unwrap());
    ...
    fn quantified_empty_group_quantifies_epsilon() {
        assert_eq!(parse("()*"), Ok(Expression::Epsilon.star()));
    }

    #[test]
    fn zero_repetition_is_allowed() {
        let expected = single('a').repeated(Quantifier::exactly(0));
    ...
    fn a_span_covers_the_construction_at_fault() {
        ...
        for (pattern, at_fault) in faults {
            let error = Parser::new(pattern).parse().unwrap_err();
            assert_eq!(
    ...
    fn a_span_names_the_help_that_corrects_the_pattern() {
        let error = Parser::new("a{5,2}").parse().unwrap_err();
    ...
    fn a_failure_that_the_author_cannot_correct_gives_no_help() {
        let error = Parser::new(r"\x{2").parse().unwrap_err();
    ...
    fn no_pattern_makes_the_parser_panic_or_misreport_a_span() {
        ...
        for length in 1..=MAX_LENGTH {
            for mut encoded in 0..ALPHABET.len().pow(length) {
                ...
                if let Err(error) = Parser::new(&pattern).parse() {
                    assert!(
    ...
    fn date_pattern() {
        ...
        let expected = digits()
            .repeated(Quantifier::exactly(4))
            .concat(single('-'))
            .concat(digits().repeated(Quantifier::exactly(2)))
            .concat(single('-'))
            .concat(digits().repeated(Quantifier::exactly(2)));
